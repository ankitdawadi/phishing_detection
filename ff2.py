...
def web_traffic(url):
    try:
        rank = bs4.BeautifulSoup(requests.get("http://data.alexa.com/data?cli=10&dat=s&url=" + url)).find("REACH")[
            'RANK']
        rank = int(rank)
        if rank < 100000:
            return -1
...
def main(url):
    ...
    check = [[using_ip(url), long_url(url), shortening_services(url), having_at(url), redirecting_double_slash(url),
              ...
              abnormal_url(url, subdomain, domain, suffix), redirects(url), onmouseover(url, soup),
              disable_right_click(url, soup),
              pop_up(url, soup), iframe(url, soup), age_of_domain(url, subdomain, domain, suffix),
              dns_record(url, subdomain, domain, suffix),
